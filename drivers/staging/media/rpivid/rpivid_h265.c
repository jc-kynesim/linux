...
struct rpivid_dec_state {
	...
	__u8 column_width[member_size(struct v4l2_ctrl_hevc_pps,
			  column_width_minus1)];
	__u8 row_height[member_size(struct v4l2_ctrl_hevc_pps,
				    row_height_minus1)];
...
static int ctb_to_tile(unsigned int ctb, unsigned int *bd, int num)
{
	...
	for (i = 1; ctb >= bd[i]; i++)
		; // bd[] has num+1 elements; bd[0]=0; see hevc_ps.c
	return i - 1;
}
...
static void WriteProb(struct rpivid_dec_env *const de,
...
static void WriteScalingFactors(struct rpivid_dec_env *const de)
{
	int i;
...
static void WriteBitstream(struct rpivid_dec_env *const de,
			   const struct rpivid_dec_state *const s)
{
...
static void write_slice(struct rpivid_dec_env *const de,
			const struct rpivid_dec_state *const s, const unsigned int slice_w,
...
static void new_entry_point(struct rpivid_dec_env *const de,
			    const struct rpivid_dec_state *const s, const int do_bte,
...
{
	int last_x =
		s->ctb_addr_ts_to_rs[ctb_addr_ts - 1] % de->PicWidthInCtbsY;
	int last_y =
		s->ctb_addr_ts_to_rs[ctb_addr_ts - 1] / de->PicWidthInCtbsY;
...
static void wpp_pause(struct rpivid_dec_env *const de, int ctb_row)
{
	...
	p1_apb_write(de, RPI_MODE,
		     ctb_row == de->PicHeightInCtbsY - 1 ? 0x70000 : 0x30000);
	p1_apb_write(de, RPI_CONTROL, (ctb_row << 16) + 2);
}

static void wpp_end_previous_slice(struct rpivid_dec_env *const de,
				   const struct rpivid_dec_state *const s, int ctb_addr_ts)
{
			    ...
			    const int reset_qp_y, const int ctb_addr_ts)
{
	...
	write_slice(de, s, slice_w,
		    ctb_row == de->PicHeightInCtbsY - 1 ? slice_h : ctb_size);
	...
	p1_apb_write(de, RPI_MODE,
		     ctb_row == de->PicHeightInCtbsY - 1 ? 0x60001 : 0x20001);
	p1_apb_write(de, RPI_CONTROL, (ctb_col << 0) + (ctb_row << 16));
}
				     ...
				     const struct rpivid_dec_state *const s)
{
	...
	static const u32 scaling_factor_offsets[4][6] = {
		...
		{ 0x0000, 0x0010, 0x0020, 0x0030, 0x0040,
		  0x0050 }, // SID0 (4x4)
		{ 0x0060, 0x00A0, 0x00E0, 0x0120, 0x0160,
		  0x01A0 }, // SID1 (8x8)
		{ 0x01E0, 0x02E0, 0x03E0, 0x04E0, 0x05E0,
		  0x06E0 }, // SID2 (16x16)
		{ 0x07E0, 0x0BE0, 0, 0, 0, 0 }
	}; // SID3 (32x32)
...
static void rpivid_h265_setup(struct rpivid_ctx *ctx, struct rpivid_run *run)
{
	...
	if (de && de->state != RPIVID_DECODE_END) {
		...
		de->rpi_config2 =
			// BitDepthY
			((s->sps.bit_depth_luma_minus8 + 8) << 0) |
			 // BitDepthC
			((s->sps.bit_depth_chroma_minus8 + 8) << 4) |
			 // BitDepthY
			((s->sps.bit_depth_luma_minus8 != 0) << 8) |
			// BitDepthC
			((s->sps.bit_depth_chroma_minus8 != 0) << 9) |
			(s->log2_ctb_size << 10) |
			((!!(s->pps.flags & V4L2_HEVC_PPS_FLAG_CONSTRAINED_INTRA_PRED))
							<< 13) |
			((!!(s->sps.flags & V4L2_HEVC_SPS_FLAG_STRONG_INTRA_SMOOTHING_ENABLED))
							<< 14) |
			((!!(s->sps.flags & V4L2_HEVC_SPS_FLAG_SPS_TEMPORAL_MVP_ENABLED))
							<< 15) |
			...
			((!!(sh->flags &
			   V4L2_HEVC_SLICE_PARAMS_FLAG_SLICE_TEMPORAL_MVP_ENABLED))
							<< 19) |
			((!!(s->sps.flags & V4L2_HEVC_SPS_FLAG_PCM_LOOP_FILTER_DISABLED))
							<< 20) |
...
static struct rpivid_dec_state *dec_state_new(void)
{
	struct rpivid_dec_state *s = kzalloc(sizeof(*s), GFP_KERNEL);
